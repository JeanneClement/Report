...
plot(W[,1],summary(MCMC.vl1)[[1]][,"Mean"],xlab="W1 fixé", ylab= "W1 estimé",
     main="W1")
title("Prédicteurs non mesurés", outer = T)
abline(a=0,b=1,col='red')
...
mod_null_probit <- jSDM::jSDM_binomial(presences=c(Y_probit), trials=rep(1,nsite*nsp), suitability=~1, data =data.frame(Y_probit=c(Y_probit),visits=c(1,nsite*nsp)), 
                                       burnin = 35000, mcmc = 5000, thin = 5,
                                       beta_start=0, mubeta = 0, Vbeta = 1e+06, seed = 1234, ropt = 0.44, verbose = 1)
null_dev_probit <- mean(mod_null_probit$mcmc[,"Deviance"])
exp_dev_probit <- dev_probit/null_dev_probit 
### RMSE
...
exp_dev_logit <- dev_logit/null_dev_logit 
##RMSE
