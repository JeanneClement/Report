...
subtitle: "Comité de suivi de thèse : 2ème année"
...
\vspace{-0.3cm}
\begin{figure}[H]
\caption[Données utilisées pour ajuster un JSDM]{Les données de présence/absence ou d'abondance des espèces ($Y$), les données bio-climatiques sur chaque site d'inventaire ($X$) et les mesures de traits pour pour chaque espèce considérées ($T$).}
\vspace{-0.4cm}
  \centering
\includegraphics[height=0.35\textheight]{Illustrations/datasets2.pdf}
\end{figure}
...
- $\alpha_i$ représente l'effet aléatoire ou fixe du site $i$ et $V_{\alpha} \sim \mathcal {IG}(\text{shape}, \text{rate})$ dans le cas où l'effet site est aléatoire. 
...
De plus, pour assurer l’identifiabilité du modèle, les valeurs de $\Lambda$ sont contraintes à des valeurs strictement positives sur la diagonale et nulles au dessus de celle-ci d’après l’article @Warton2015, $\Lambda$ est ainsi supposée être triangulaire inférieure. En effet la distribution *a priori* des facteurs latents est définie comme suit : 

$$\lambda_{jl} \sim
\begin{cases}
\mathcal{N}(\mu_{\lambda},V_{\lambda}) & \text{si } l < j \\
\mathcal{N}(\mu_{\lambda},V_{\lambda}) \text{ tronquée à gauche par } 0 & \text{si } l=j \\
\delta(\lambda_{jl}) \text{ (Dirac distribution) } & \text{si } l>j
\end{cases}$$
...
\caption[Un résumé graphique du modèle hiérarchique bayésien. Dans ce graphe acyclique dirigé (DAG), les cases orange font référence aux données, les cercles bleus aux paramètres à estimer, et les flèches aux relations fonctionnelles décrites à l'aide de distributions statistiques]{Un résumé graphique du modèle hiérarchique bayésien. Dans ce graphe acyclique dirigé (DAG), les cases orange font référence aux données, les cercles bleus aux paramètres à estimer, et les flèches aux relations fonctionnelles décrites à l'aide de distributions statistiques}
  \centering
\includegraphics[height=0.3\textheight]{Illustrations/DAG-jSDM-rand.png}
\end{figure}
...
* Dans le cas du modèle logit on suppose que $y_{ij} \ | \theta_{ij} \sim \mathcal{B}inomial(n_i,\theta_{ij})$, avec
...
D'une part, nous avons utilisé le package R `Hmsc` 3.0-11, pour "Hierarchical Modelling of Species Communities", qui est une approche basée sur un modèle pour analyser les données écologiques des communautés ([@Ovaskainen2017]). D'autre part, nous avons utilisé le package R `boral` 2.0 [@Hui2016], pour "Bayesian Ordination and Regression Analysis", qui fonctionne avec `JAGS` ("Just Another Gibbs Sampler") un programme de simulation de modèles bayésiens hiérarchiques utilisant des méthodes MCMC, implémenté en C++. Ces packages et le package `jSDM` 0.2.1 permettent d'ajuster différents JSDMs, nous avons donc pu comparer la pertinence des résultats obtenus par chacun d'eux, leurs performances de calcul et l'efficacité des échantillonneurs sur différents jeux de données. Avec chacun de ces trois packages, nous avons effectué des régressions binomiales probit à partir d'un jeu de données simulé ainsi que quatre jeux de données de présence-absence, utilisés dans une étude comparative de JSDM récente par @Wilkinson2019. Nous avons également effectué des régressions log-linéaires de Poisson à partir de deux jeux de données d'abondance [@Choler2005 ; @Borcard1994]. L'ajustement des modèles a été effectué sur le même ordinateur de bureau avec les spécifications suivantes : Ubuntu 20.04.4 OS, i5-1145G7 CPU @ 2.60GHz × 8, 15.4GB RAM) afin de comparer les performances de calcul.


## Comparaison de la pertinence des résultats obtenus et des temps de calcul nécessaires avec chacun des packages

\begin{table}[H]
\caption{Dimensions des ensembles de données utilisés (n.sites et n.espèces), nombre d'observations (n.obs=n.site $\\times$ n.espèces), nombre de covariables, traits spécifiques et axes latents considérés (n.col.X, n.traits et n.latent), nombre de paramètres à estimer (n. param) en effectuant n.mcmc itérations, le temps de calcul en secondes nécessaire pour ajuster le modèle à partir de chacun des ensembles de données et la déviance calculée à partir des paramètres estimés avec chacun des packages comme suit : $D=-2\sum\limits_{i=1}^I\sum\limits_{j=1}^J\log(\mathbb {P}(y_{ij}|\beta_j,\lambda_j, W_i))$. L'erreur quadratique moyenne (RMSE) est calculée pour les probabilités de présence $\theta_{ij}$ sur l'ensemble des données simulées. La sensibilité, la spécificité et la "True Skill Statistic" ($TSS= sensibilité + spécificité -1$) ont été calculées pour chaque ensemble de données, en tenant compte de la matrice de présence-absence prédite : $\hat{Y}$ telle que $\hat{y}_{ij}=1$ si la probabilité d'occurrence estimée correspondante $ \hat{\theta}_{ij} \geq \tau_i$ et $\hat{y}_{ij}=0$ sinon, où $ \tau_i$ représente le seuil de présence pour le site $i$, défini par la valeur de probabilité d'occurrence estimée la plus faible parmi les $n_i$ probabilités de présence d'espèces les plus élevées, où $n_i$ correspond au nombre d'espèces observées sur le site $i$.} 
\label{tab:comparison-time-accuracy}
\centering
\includegraphics[width=15cm, height=30cm]{Illustrations/comparison.pdf}
\end{table}
...
load("~/Documents/Articles/jSDM_functionalities/Application/jSDM_boral_Hmsc_files/jSDM-boral-Hmsc-comparison.rda")

T_boral=as.numeric(c(T_boral_sim, T_boral_Mosquitos, T_boral_Eucalypts, T_boral_Frogs, T_boral_Fungi, T_boral_Aravo, T_boral_Mites))
T_Hmsc=as.numeric(c(T_Hmsc_sim, T_Hmsc_Mosquitos, T_Hmsc_Eucalypts, T_Hmsc_Frogs, T_Hmsc_Fungi,
             T_Hmsc_Aravo, T_Hmsc_Mites))
T_jSDM=as.numeric(c(T_jSDM_sim, T_jSDM_Mosquitos, T_jSDM_Eucalypts, T_jSDM_Frogs,T_jSDM_Fungi,
             T_jSDM_Aravo, T_jSDM_Mites))
min_ratio_jSDM_Hmsc <- round(min(T_Hmsc/T_jSDM),1)
max_ratio_jSDM_Hmsc <- round(max(T_Hmsc/T_jSDM),1)
min_ratio_jSDM_boral <- round(min(T_boral/T_jSDM),1)
max_ratio_jSDM_boral <- round(max(T_boral/T_jSDM),1)
min_ratio_boral_Hmsc <- round(min(T_boral/T_Hmsc),1)
max_ratio_boral_Hmsc <- round(max(T_boral/T_Hmsc),1)
```


On constate que `jSDM` est **`r min_ratio_jSDM_boral` à `r max_ratio_jSDM_boral`** fois plus rapide que `boral` (`JAGS`)
et **`r min_ratio_jSDM_Hmsc` à `r max_ratio_jSDM_Hmsc`**  fois plus rapide que `Hmsc`.
...
Les spécificitées, sensitivités et TSS obtenus avec `jSDM` sont aussi proches de 1 que ceux calculés avec `boral` et `Hmsc` ce qui suggère que la pertinence des résultats de `jSDM` est équivalente voire supérieure à celle des résultats obtenus avec les autres packages. 
...
## Comparaison de l'efficacité d'échantillonnage 

D'autre part on compare l'efficacité des échantillonneurs utilisés dans les différents packages.
Pour ce faire on calcule le nombre de valeurs efficaces échantillonnées par seconde par chacun des packages et pour chacun des paramètres du modèle, afin d'évaluer leur efficacité d'échantillonnage pas seulement en fonction du temps de calcul nécessaire à l'ajustement des modèles mais aussi en prenant en compte l'auto-corrélation dans les chaînes MCMCs de paramètres renvoyées. 

La taille d'échantillon effective (ESS) est calculée pour chaque paramètre à l'aide de la fonction \href{https://www.rdocumentation.org/packages/coda/versions/0.19-4/topics/effectiveSize}{`effectiveSize`} du package `coda`.

\begin{table}[H]
\caption{Le nombre de valeurs efficaces échantillonnées par seconde, par chaque package. Nous supposons que le temps d'échantillonnage (T.sample) correspond à la moitié du temps de calcul (voir Tableau ~\ref{tab:comparison-time-accuracy}) car le même nombre d'itérations d'échantillonnage (n.sample) et de burn-in (n.burnin) ont été effectuées pour ajuster les modèles. Pour réduire l'auto-corrélation des MCMCs, seule une itération d'échantillonnage sur n.thin a été retenue et un échantillon de taille : sample.size, est retourné pour chaque paramètre. Pour obtenir le nombre de valeurs efficaces échantillonnées par seconde, au total ou pour l'intercept espèce $\beta_0$, l'effet espèce $\beta$, les charges factorielles $\lambda$ et les variables latentes $W$, la somme des ESS calculées pour tous les paramètres, ou la moyenne des ESS pour chaque type de paramètres, est divisée par le temps d'échantillonnage en secondes.}
\centering
\includegraphics[height=0.8\textheight]{Illustrations/neff.pdf}
\label{tab:neff}
\end{table}

On constate que le nombre total de valeurs efficaces échantillonnées par seconde par `jSDM` est supérieur à celui de `Hmsc` pour chacun des jeux de données, l'échantillonneur de `jSDM` est donc plus efficace. Néanmoins si on s'intéresse uniquement aux facteurs latents $\lambda$,  les nombres de valeurs efficaces échantillonnées par seconde par `Hmsc` et plus important que ceux de `jSDM`, ce qui peut s'expliquer par les méthodes d'inférences qui diffèrent entre les deux packages pour ces paramètres.   

De plus `jSDM` présente un nombre total de valeurs efficaces échantillonnées par seconde plus élevé que `boral` concernant les jeux de données de présence-absence. En ce qui concerne les jeux de données d'abondance, l'échantillonneur de `boral` est légérement plus efficace que celui de `jSDM`, mais c'est le cas car il s'agit de petits jeux de données, sur des jeux de données d'abondance plus conséquents, `jSDM` deviendrait vite plus performant que `boral`. 

# Amélioration de la convergence pour l'estimation des axes latents
...
- Évaluer la convergence des paramètres associés aux axes latents et diagnostiquer l'espèce ($j$) qui se structure le plus clairement sur chaque axe ($l$) en essayant différents critères:
  - Maximise les moyennes de la valeur absolue des $\lambda$ calculées sur chaque chaîne MCMC : $\overline{| \lambda_{jl} |}.$
  - Maximise les coefficients de variation calculées sur chaque chaîne MCMC: $\frac{\overline{| \lambda_{jl} |}}{\sqrt{\mathrm{Var}(\lvert \lambda_{jl} \rvert})}.$
  - Maximise les diagnostiques de Gelman–Rubin ($\hat{R_{jl}}$) calculé entre toutes les chaînes MCMC obtenues pour chaque $\lambda$, qui évalue la convergence en comparant les varainces intra-chaînes et inter-chaînes estimées pour chaque paramètre. 
...
### Spécification de la zone géographique d'intérêt 

La zone géographique sur laquelle les données seront extraites peut être spécifiée par l'utilsateur au moyen d'un shapefile indiquant les frontières du territoire, de l'étendue de la zone définie selon le système de coordonnées spécifié pour reprojeter les données extraites, ou du code ISO 3166 correspondant s'il s'agit d'un pays. 

### Extractions de données climatiques actuelles et futures

Les données climatiques actuelles sont extraites du site  [chelsa-climate.org](https://chelsa-climate.org/) (moyennes sur la période de 1981 à 2010).
<!-- ou de [worldclim.org](https://worldclim.org/) (moyennes sur la période de 1970 à 2000). -->
Le téléchargement des données se fait uniquement à l’échelle mondiale, ce qui est un avantage pour généraliser l’acquisition à n'importe quelle zone géographique mais qui nécessite un temps de téléchargement et un espace de stockage relativement importants. 

En ce qui concerne les données climatiques futures, on laisse à l'utilisateur du package la possibilité de spécifier pour quels modèles climatiques globaux (GCMs) et pour quels scénarios socio-économiques partagés (SSPs pour "Shared Socio-economic Pathways"), il souhaite importer ces données. 
Le choix de l'utilisateur est cependant limité par la disponibilité des données climatiques futures estimées selon le GCM et le SSP choisis, sur le site [chelsa-climate.org](https://chelsa-climate.org/). Les scénarios CMIP6 pour lesquels les données sont disponibles sur le sites source correspondent aux différentes combinaisons possibles de trois SSPs (SSP1-RCP2.6, SSP3-RCP7, SSP5-RCP8.5) et de cinq GCMs proposés par les laboratoires ci dessous: 

- GFDL-ESM4, Princeton University Forrestal Campus, New Jersey, Etats
Unis d’Amérique
...
De plus les valeurs mensuelles pour les données climatiques futures, disponibles sur  [chelsa-climate.org](https://chelsa-climate.org/), sont des moyennes sur sur des périodes de 30 ans (2011-2040, 241-2070, 2071-2100).
<!-- des périodes de 20 ans (2021-2040, 241-2060, 2061-2080, 2081-2100) concernant les données de [worldclim.org](https://worldclim.org/) -->
\begin{table}[H]
\caption{Données climatiques téléchargées par la fonction \texttt{get\_chelsa\_current} and \texttt{get\_chelsa\_future}.}
\begin{tabular}{l|l|l}
\rowcolor{gray!50}
\textbf{Variable} &  \textbf{Unité} & \textbf{Source} \\
Températures mensuelles minimales & $ \celsius \times 10 $ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\ 
Températures mensuelles maximales &  $ \celsius \times 10 $ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\ 
Températures mensuelles moyennes &  $ \celsius \times 10 $ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Précipitations mensuelles & $kg.m^{-2}.mois^-1$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Couverture nuageuse &  \%  & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\ 
Déficit hydrique climatique (Thornthwaite) &  $kg.m^{-2}$ & calculé à partir des données de \\
& & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Evapotranspiration potentielle (Thornthwaite) & $kg.m^{-2}.mois^-1$ & calculée à partir des données de \\ 
& &\href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Nombre de mois secs (Thornthwaite) & mois & calculés à partir des données de \\ 
& & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Déficit hydrique climatique (Penman-Monteith) & $kg.m^{-2}$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Evapotranspiration potentielle (Penman-Monteith) & $kg.m^{-2}.mois^-1$ &  \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Nombre de mois secs (Penman-Monteith) & mois & calculés à partir des données de \\ 
& &\href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Température moyenne annuelle & $\celsius \times 10$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Température moyenne annuelle (bio1) & $\celsius \times 10$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Amplitude des températures diurnes (bio2) & $\celsius \times 10$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Isothermalité (bio3=bio2/bio7) & $\celsius \times 10$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Saisonnalité des températures (bio4) & $\celsius / 10$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Température maximale du mois le plus chaud (bio5) & $\celsius \times 10$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Température minimale du mois le plus froid (bio6) & $\celsius \times 10$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Amplitude annuelle des températures (bio7=bio5-bio6) & $\celsius \times 10$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Température moyenne du trimestre le plus humide (bio8) & $\celsius \times 10$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Température moyenne du trimestre le plus sec (bio9) & $\celsius \times 10$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Température moyenne du trimestre le plus chaud (bio10) & $\celsius \times 10$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Température moyenne du trimestre le plus froid (bio11) & $\celsius \times 10$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Cumul annuel des précipitations (bio12) &  $kg.m^{-2}.an^{-1}$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Cumul des précipitations du mois le plus humide (bio13) &  $kg.m^{-2}.mois^{-1}$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Cumul des précipitations du mois le plus sec (bio14) &  $kg.m^{-2}.mois^{-1}$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Saisonnalité des précipitations (bio15) &  $kg.m^{-2}$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Précipitations du trimestre le plus humide (bio16) &  $kg.m^{-2}.mois^{-1}$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Précipitations du trimestre le plus sec (bio17) &  $kg.m^{-2}.mois^{-1}$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Précipitations du trimestre le plus chaud (bio18) &  $kg.m^{-2}.mois^{-1}$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
Précipitations du trimestre le plus froid (bio19) &  $kg.m^{-2}.mois^{-1}$ & \href{https://chelsa-climate.org/}{chelsa-climate.org} \\
\end{tabular}
\end{table}



### Extraction de données environnementales  
...
\begin{table}[H]
\caption{Données environnementales téléchargées par la fonction \texttt{get\_env\_variables}.}
\begin{tabular}{l|l|l}
\rowcolor{gray!50}
\textbf{Variable} &  \textbf{Unité} & \textbf{Source} \\
Couvert forestier & \% & calculé à partir des données de \url{https://forestatrisk.cirad.fr} \\
Altitude & m & \url{https://srtm.csi.cgiar.org/} \\ 
Pente & degrès & calculée à partir des données de \url{https://srtm.csi.cgiar.org/} \\  
Orientation du sol (aspect) & degrès & calculé à partir des données de \url{https://srtm.csi.cgiar.org/} \\  
Rugosité & m & calculée à partir des données de \url{https://srtm.csi.cgiar.org/} \\  
Ensoleillement & $Wh.m^{-2}.jour^{-1}$ & calculé à partir des données  \url{https://srtm.csi.cgiar.org/} \\  
Distance à la forêt & m  & calculée à partir des données de \url{https://www.openstreetmap.org/} \\  
Distance à la mer & m & calculée à partir des données de \url{https://www.openstreetmap.org/} \\  
Distance aux routes & m & calculée à partir des données de \url{https://www.openstreetmap.org/} \\  
Distance aux villes/villages & m & calculée à partir des données de\url{https://www.openstreetmap.org/} \\  
Distance aux points d’eau & m & calculée à partir des données de \url{https://www.openstreetmap.org/} \\  
Zones protégées (WDPA) & catégories & \url{https://www.protectedplanet.net/en} \\ 
Densité de population & habitants/km$^2$ & \href{https://data.worldpop.org/GIS/Population/Global_2000_2020_Constrained/2020/}{https://data.worldpop.org/GIS/Population} \\
Données pédologiques & catégories & \url{https://soilgrids.org} \\
(type de sol et leur propriétés & & \\ 
chimiques et physique)  & & \\ 
\end{tabular}
\end{table}

### Efficacité de l'extraction et du formatage des données 
...
## Éstimation de la biodiversité à Madagascar 
...
### Ajustement d'un JSDM à partir de ces données  
 ...
 On ajuste un modèle joint de distribution des espèces de fonction de lien probit en considérant deux variables latentes et un effet site aléatoire à partir des données décrites précédemment, à l'aide de la fonction [jSDM_binomial_probit](https://ecology.ghislainv.fr/jSDM/reference/jSDM_binomial_probit.html) du package `jSDM`.
<!--en effectuant $80 000$ itérations dont $40 000$ de burn-in et on retient $N_{samp}=1 000$ valeurs pour chaque paramètre du modèle que l'on va représenter en fonction du nombre d'itérations effectuées afin d'évaluer la convergence de l'algorithme de Gibbs.   -->
...
Après avoir ajusté le JSDM, d'après les articles  @Warton2015 et @Tobler2019, la  **matrice de corrélation résiduelle** $R=(R_{ij})^{i=1,\ldots, J}_{j=1,\ldots, J}$ peut être obtenue de la manière suivante : 
 $$\Sigma_{gj} = \begin{cases}
 \lambda_g'.\lambda_j & \text{ if } g \neq j \\
  \lambda_g'.\lambda_j^T & \text{ if } g=j
 \end{cases}$$, on calcule ensuite les corrélation à partir des covariances :
  $$R_{g,j} = \frac{\Sigma_{gj}}{\sqrt{\Sigma _{gg}\Sigma _{jj}}}$$.
...
Cette représentation des associations entres espèces permet d'observer les corrélations positives ou négatives entre les espèces qui sont interprétables en terme d'influence positive ou négative de la présence d'un espèce sur la probabilité d'occurrence d'une autre. On constate sur la figure de gauche que les corrélations résiduelles prépondérantes sont plus nombreuses entre les espèces les plus abondantes, c'est à dire celles observées sur un grand nombre de sites. 
De plus on peut voir sur la de droite que les espèces dont la distribution est influencée principalement par le déficit hydrique (cwd) et celles dont l'effet environnemental principal et la saisonnalité des précipitations (sais_prec) sont fortement corrélées. 

### Estimation de la richesse spécifique pour les placettes d'inventaire et comparaison à celle observée

La richesse spécifique aussi appelée diversité $\alpha$ reflète le nombre d'espèces coexistant dans un milieu donné, on l'estime en additionnant les probabilités de présence estimée [@Scherrer2020].  
...
J'ai représenté les cartes suivantes de biodiversité $\alpha$ et $\beta$ afin d'illustrer la méthode mise en oeuvre pour les obtenir mais elles peuvent présenter des incohérences car elles sont basées sur les résultats de l'interpolation par RST qui nécessite d'être améliorée.

Nous avons additionné les probabilités de présence interpolées pour chacune des espèces afin d'estimer la richesse spécifique.
...
(ref:cap-forest-type) Types de forêts à Madagascar de l'article issus de l'article @Vieilledent2018.

(ref:cap-div-beta) Turnover estimé des espèces à l'échelle de Madagascar (gauche) et turnover des espèces à Madagascar estimé restreint à la couverture forestière restante en 2000 de l'article @Vieilledent2018, pour tenir compte de la déforestation sur l'île (droite).

```{r diversity-beta, echo=FALSE, fig.show = "hold", fig.pos = "H", out.height=c("30%","27%"), fig.col=1, fig.align = "center",  eval=TRUE, fig.cap="Diversité $\\beta$ interpolée sur l'ensemble de l'île et restreinte au couvert forestier connu en 2000 et types de forêts à Madagascar.", fig.subcap=c("(ref:cap-div-beta)","(ref:cap-forest-type)")}
knitr::include_graphics("figure-html/plot-diversity-beta-1.png") 
knitr::include_graphics("figure-html/mada_forest_type.jpg")
```

Nous pouvons constater que la carte estimée de la diversité $\beta$ sur la figure \@ref(fig:diversity-beta).a concorde avec la carte des types de forêts de Madagascar de l'article @Vieilledent2018 sur la figure \@ref(fig:diversity-beta).b. En effet, les communautés d'espèces représentées par des nuances de bleu et de vert correspondent à la forêt humide de l'est, les communautés représentées par des nuances de rouge correspondent à la forêt sèche de l'ouest et les communautés en magenta correspondent à la forêt épineuse du sud. De plus, la carte de la diversité $\beta$ que nous obtenons permet d'identifier des communautés d'espèces sensiblement différentes au sein de ces trois types de forêts. Par exemple, nous pouvons clairement identifier trois communautés d'arbres différentes (en bleu, vert et noir) au sein de la forêt humide orientale.
...
## Classification des communautés d’arbres en Nouvelle-Calédonie 
...
### Ajustement d'un JSDM en Nouvelle Calédonie

Dans le cadre de son stage, Pierre a particpé au développement du package R `gecevar`, Pierre a utilisé le package R `jSDM`, que j’ai développé, afin d’ajuster un modèle joint de distribution des espèces en Nouvelle Calédonie, à partir des données climatiques et environnementales obtenues à l’aide du package `gecevar`, ainsi que de données d’inventaire forestier répertoriant un très grand nombre d’espèces sur de nombreux sites d’inventaires. 


\begin{figure}[H]
\caption{Variables explicatives du modèle et les ordres considérés.}
  \centering
\includegraphics[height=0.15\textheight]{Illustrations/X_NC.png}
\end{figure}

\begin{figure}[H]
\caption{Ajustement d'un JSDM à partir des données disponibles en Nouvelle Calédonie.}
  \centering
\includegraphics[height=0.13\textheight]{Illustrations/JSDM_NC.png}
\end{figure}
...
### Obtention de cartes de biodiveristé en Nouvelle Calédonie 
...
\begin{figure}[H]
\caption{Richesse spécifique estimée en Nouvelle Calédonie, restreinte au couvert forestier restant en 2000}
  \centering
\includegraphics[height=0.25\textheight]{Illustrations/sp_rich.png}
\end{figure}
\begin{figure}[H]
\caption{Diversité $\beta$ estimée en Nouvelle Calédonie, restreinte au couvert forestier restant en 2000}
  \centering
\includegraphics[height=0.25\textheight]{Illustrations/div_beta_NC.png}
\end{figure}
...
# Perspectives de développement du package `jSDM`

## Fonctionnalités en cours de développement

### Ajuster un modèle avec des effets espèces aléatoires

Dans le cas où on considère que les effets espèces $\beta$ sont aléatoire, leur distribution *a priori* est de la forme : $\beta_j \sim  \mathcal{N}_{p+1}(\mu_{\beta},V_{\beta})$ et on suppose que $V_{\beta} \sim \mathcal{W}ishart^{-1}(M,\nu)$.  
...
### Ajuster un JSDM à partir d'une variable réponse continue 
...
Dans ce cas on suppose que $y_{ij} \sim \mathcal{N}(\mu_{ij},\sigma^2)$, pour des données continues et on considère le modèle linéaire à variables latentes suivant : $$\mu_{ij}=\alpha_i + X_i\beta_j + W_i\lambda_j + \epsilon_{ij}$$
où $\epsilon_{ij} \sim \mathcal{N}(0,\sigma^2)$ et  $\sigma$ est un paramètre de dispersion à estimer. 

Pour ajuster ce modèle, on utilise un échantilloneur de Gibbs associé aux formules des priors conjugués, de la même façon que pour le modèle probit. 
...
## Estimation du nombre d'axes latents à prendre en compte
