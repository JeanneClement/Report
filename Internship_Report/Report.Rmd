...
L’algorithme utilisé pour estimer les paramètres du modèle probit est donc le suivant :
\begin{itemize}
...
Cet algorithme appartient aux méthode MCMC et permet d’obtenir une réalisation du paramètre $\theta=(\theta_1,\ldots,\theta_m)$ selon leurs distributions conditionnelles *a posterirori* $\uppi(\theta_i \ | \ \theta_1,\dots,\theta_{i-1},\theta_{i+1},\ldots,\theta_m, x)$, pour $i =1,\ldots,m$ connues à une constante multiplicative près.  
...
result[2,] <- c(300 ,167,455,104,438)
result[3,] <- c(2,13 ,7 ,3,12 )
result[4,] <- c((2+1)*100+2*100+2*300+300, (13+1)*16+2*16+2*167+167, (7+1)*12+2*12+3*455, (3+1)*9+2*9+3*104, (12+1)*11+2*11+3*438)
...
```{r time-jSDM-boral,echo=F, include=T}
library(dplyr)
load("data/jSDM-boral-comparison.rda")
result <- data.frame(matrix(NA,2,5),row.names=c("boral","jSDM "))
colnames(result) <- c("Simulation","Moustiques","Eucalyptus","Grenouilles","Champignons")
result[1,]=round(as.numeric(c(T_boral_sim, T_boral_Mosquitos, T_boral_Eucalypts, T_boral_Frogs, T_boral_Fungi),units="mins"),1)
result[2,]=round(as.numeric(c(T_jSDM_sim, T_jSDM_Mosquitos, T_jSDM_Eucalypts, T_jSDM_Frogs, T_jSDM_Fungi),units="mins"),1)
...
kable(result, row.names=T, digits=4,booktabs=TRUE,align = 'c') %>%
		kableExtra::kable_styling(latex_options=c("HOLD_position","striped"), full_width=FALSE)
```
\end{table} 
Le RMSE associé à jSDM est équivalent à celui de boral ce qui indique que les résultats obtenus avec jSDM sur le jeu de données simulé sont aussi proches de ceux attendus que les paramètres estimés avec boral.
...
result <- data.frame(matrix(NA,2,5),row.names=c("boral", "jSDM"))
colnames(result) <- c("Simulation","Moustiques","Eucalyptus","Grenouilles","Champignons")
result[1,] <- c(Deviance_boral_sim, Deviance_boral_Mosquitos, Deviance_boral_Eucalypts, Deviance_boral_Frogs, Deviance_boral_Fungi)
result[2,] <-  c(Deviance_jSDM_sim, Deviance_jSDM_Mosquitos, Deviance_jSDM_Eucalypts, Deviance_jSDM_Frogs, Deviance_jSDM_Fungi)
kable(result, row.names=T, digits=0,booktabs=TRUE,align = 'c') %>%
		kableExtra::kable_styling(latex_options=c("HOLD_position","striped"), full_width=FALSE)
```
\end{table} 
...
\centering
\includegraphics[height=0.45\textheight, width=0.4\textwidth]{Illustrations/jSDM-boral-Mosquitos-1.pdf} 
\includegraphics[height=0.45\textheight, width=0.4\textwidth]{Illustrations/jSDM-boral-Mosquitos-2.pdf}\\ 
\includegraphics[height=0.45\textheight, width=0.4\textwidth]{Illustrations/jSDM-boral-Mosquitos-3.pdf}  
\includegraphics[height=0.45\textheight, width=0.4\textwidth]{Illustrations/jSDM-boral-Mosquitos-4.pdf}\\ 
\includegraphics[height=0.45\textheight, width=0.4\textwidth]{Illustrations/jSDM-boral-Mosquitos-5.pdf} 
\end{figure} 
\begin{figure}[H]
...
library(dplyr)
cv_alphas<-read.csv("data/cv_alphas.csv")
