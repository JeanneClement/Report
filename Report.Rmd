...
(Au cas où je rajoute la partie comparaison boral jSDM : 
...
En ce qui concerne l'effet site aléatoire $(\alpha_i)_{i=1,\dots,I}$, on pose $Z^*_{ij} = Z_{ij} - D_iP_j = \alpha_i + \epsilon_{ij}$, avec $D_i =(1,X_{i1},\ldots,X_{ip},W_{i1},\ldots,W_{iq})$. \smallskip   
...
\newpage
### Echantilloneur de Gibbs et priors conjugués
...
\begin{itemize}
\item Calcule de $\mathrm{logit}(\theta_{ij})= \alpha_i + \beta_{j0} + X_i\beta_j + W_i\lambda_j$.
\item Calcule de $\theta_{ij}= \dfrac{1}{1+\exp\left(-\mathrm{logit}(\theta_{ij})\right)}$.
...
On considérera deux prédicteurs non mesurés.  
...
\end{aligned}$$
Les prédicteurs non mesurés $W_{il} \sim \mathcal{N}(0,1) \text{ pour } i=1,\ldots,500 \text{ et } l=1,2$.  
...
On effectue $40 000$ itérations au total dont $35 000$ de burnin puis $5 000$ durant lesquelles les fonctions implémentées enregistrent les paramètres estimés à partir des données simulées précédemment, toutes les $5$ itérations et retournent un échantillon de $N_{samp}=1 000$ valeurs pour chacun des paramètres du modèle considéré.   
...
Le pourcentage de déviance expliquée est défini par $1-\dfrac{D}{D_0}$ avec $D=-2(l(\widehat{\theta})-l_s)$ la déviance du modèle considéré avec $l_s$ la log-vraisemblance du modèle saturé comportant autant de paramètres qu'il y a d'observations et dont l'ajustement est supposé parfait par conséquent dans le cas de données binaires, les probabilités ajustées prenant les valeurs $0$ ou $1$ en fonction de celles de $Y$, on a $l_s=0$ car la vraisemblance des données observées sous le modèle saturé est de $1$.\\
...
On ajuste le modèle nul avec la fonction jSDM\_binomial() du package jSDM pour le modèle logit et avec hSDM\_probit\_block() pour le modèle probit.\\
...
results <- data.frame("Temps"=c(T_probit,T_logit), NRMSE=c(NRMSE_probit,NRMSE_logit), D=c(1-exp_dev_probit,1-exp_dev_logit)*100)
colnames(results)<- c("Temps de calcul (mins)","NRMSE","Déviance expliquée (%)")
rownames(results)<- c("Modèle probit","Modèle logit")
knitr::kable(results, row.names=T, digits=1,booktabs=TRUE) %>%
		kableExtra::kable_styling(latex_options=c("HOLD_position","striped"), full_width=FALSE)
```
\end{table} 

Afin d'ajuster un modèle joint de distribution des espèces à partir des données collectées à Madagascar, on utilisera donc la fonction implémentée pour le modèle probit qui présente un pourcentage de déviance expliquée et un RMSE indiquant que les paramètres estimés sont en relativement en acoord avec les données et proches des valeurs attendues ainsi qu'un temps de calcul bien inférieur à celui de la fonction ajustant le modèle logit. 
...
On dispose d'inventaires forestiers réalisés sur $751$ sites de l'île de Madagascar et répertoriant la présence ou l'absence de $483$ espèces végétales sur chacun de ces sites. 
...
Les températures (temp) et précipitations (temp) moyennes annuelles qui sont exprimées respectivement en $\degre C$ et millimètres.\\
...
knitr::kable(head(ord_data_clim), row.names=F, digits=3,booktabs=TRUE) %>%
		kableExtra::kable_styling(latex_options=c("HOLD_position","striped"), full_width=FALSE)
```
\end{table}
...
$X_i=(1,\mathrm{temp}_i, \mathrm{prec}_i,\mathrm{sais\_temp}_i, \mathrm{sais\_prec}_i,  \mathrm{cwd}_i,\mathrm{temp}_i^2, \mathrm{prec}_i^2,\mathrm{sais\_temp}_i^2, \mathrm{sais\_prec}_i^2, \mathrm{cwd}_i^2)$ pour $i=1,\ldots,751$.  

Les coordonnées des sites seront utilisées par la suite dans le cadre de l'interpolation spatiale des résultats. 

## Estimation des paramètres et représentation des résulats 
...
np<-ncol(X)
write.csv(X, "data/X.csv", row.names = F)
...
write.csv(params_sites, file = "data/params_sites.csv",row.names = F)

## probit_theta_pred
write.csv(mod_all$probit_theta_pred, file = "data/probit_theta_pred.csv", row.names = F)
```

On ajuste un modèle joint de distribution des espèces de fonction de lien probit et en considérant deux variables latentes à l'aide de la fonction *jSDM\_probit\_block()* du package jSDM à partir des données décrites précédemment, en effectuant $100 000$ itérations dont $90 000$ de burnin et on retient $N_{samp}=1 000$ valeurs pour chaque paramètre du modèle que l'on va représenter en fonction du nombre d'itérations effectuées afin d'évaluer la convergence de l'algorithme de Gibbs.  

On représente également une estimation de la densité pour chaque échantillon obtenu qui devrait correspondre aux distributions *a posteriori* définies et donc être de forme gaussienne.   

```{r plot-results-Mada, cache=T,echo=F,include=F,eval=F}
## alpha_i of the first two sites
plot(coda::as.mcmc(mod_all$mcmc.alpha[,1:2]))
title(main="Effet site aléatoire alpha pour les deux premiers sites")
## Valpha
par(mfrow=c(1,2))
coda::traceplot(mod_all$mcmc.Valpha)
coda::densplot(mod_all$mcmc.Valpha)
title(main=" Variance Valpha associée à l'effet site")

## beta_j of the first two species
par(mfrow=c(np,2))
for (j in 1:2) {
  for (p in 1:np) {
      coda::traceplot(coda::as.mcmc(mod_all$mcmc.sp[[paste0("sp_",j)]][,p]))
      coda::densplot(coda::as.mcmc(mod_all$mcmc.sp[[paste0("sp_",j)]][,p]), 
main = paste(colnames(mod_all$mcmc.sp[[paste0("sp_",j)]])[p],
", species : ",j))
  }
}
title(main="Effet espèce fixe beta pour les deux premières espèces")
...
for (j in 1:2) {
  for (l in 1:n_latent) {
      coda::traceplot(coda::as.mcmc(mod_all$mcmc.sp[[paste0("sp_",j)]][,np+l]))
      coda::densplot(coda::as.mcmc(mod_all$mcmc.sp[[paste0("sp_",j)]][,np+l]), 
      main = paste(colnames(mod_all$mcmc.sp[[paste0("sp_",j)]])[np+l],
      ", species : ",j))
  }
  }
title(main="Effet espèce fixe lambda pour les deux premières espèces")

## Latent variables W_i for the first two sites
par(mfrow=c(1,2),oma=c(1, 0, 1.4, 0))
for (l in 1:n_latent) {
  plot(mod_all$mcmc.latent[[paste0("lv_",l)]][,1:2], main = paste0("W_", l))
}
title(main="Prédicteurs non mesurés", outer=T)
## probit_theta
par (mfrow=c(1,2))
hist(mod_all$probit_theta_pred, main = "probit(theta) estimé", xlab ="probit(theta) estimé")
## Deviance
plot(mod_all$mcmc.Deviance,main = "Deviance")
```

\begin{figure}[H]
 \caption[Représentation spatiale des prédicteurs non mesurés estimés pour chaque site]{Représentation spatiale des prédicteurs non mesurés estimés pour chaque site}
  \label{fig:condSimApp}
  \centering
...
\begin{figure}[H]
 \caption[Représentation spatiale des effets sites estimés]{Représentation spatiale des effets sites estimés}
  \label{fig:condSimApp}
  \centering
\includegraphics[width=1.0\textwidth,height=0.5\textheight]{Illustrations/mada_site_effect.png}
\end{figure} 
\end{minipage}
\begin{minipage}[r]{0.5\textwidth}
\begin{figure}[H]
 \caption[Représentation spatiale de la richesse spécifique estimée pour chaque site]{Représentation spatiale de la richesse spécifique estimée pour chaque site par $\sum\limits_ {j=1}^{483} \widehat{\theta_{ij}}$}
  \label{fig:condSimApp}
  \centering
\includegraphics[width=1.0\textwidth,height=0.5\textheight]{Illustrations/mada_specific_richness.png}
\end{figure} 
\end{minipage}

## Comparaison de effets sites obtenus avec différentes méthodes d'interpolation spatiale
...
\end{figure}
\begin{figure}[H]
 \caption[Interpolation spatiale des prédicteurs non mesurés par krigeage ordinaire]{Interpolation spatiale des prédicteurs non mesurés par krigeage ordinaire}
  \label{fig:condSimApp}
  \centering
